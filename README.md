## August 22, 2026 Control-5 correction: live run-alias observability gap

Production remains **FAIL/DEGRADED**, not healthy. The strict GET-only control found live `/api/runs?trigger=daytrading_cron` and `/api/runs?trigger=reconciliation_cron` returning empty rows while canonical `cron` and `reconcile_cron` filters returned data; live `/health` and `/api/config` also remain `1.0.0` and `2.4.0` versus the tested local release `2.6.0`.

The reliability-only correction preserves canonical stored triggers and adds an explicit response-only `trigger_alias` for alias-filtered reads, so clients can distinguish requested aliases without rewriting history or changing scheduler dispatch. No caps, schedules, sizing, edge gates, order behavior, broker authority, migrations, or broker-mutating endpoints changed. The durable analyzed/filtered run-count gap, absent production `rawEdgeBps` producer, stale daytrading/swing delivery, exact fill-lot accounting gap, and three-vs-four schedule evidence remain separate follow-ups.

Validation required for this correction: focused dashboard/read-only and release/crypto regressions, full `bun test`, TypeScript, diff check, and Wrangler dry-run. Deployment is authorized only as a reliability maintenance action after validation, but current Wrangler identity is unauthenticated and the required `CLOUDFLARE_API_TOKEN` is unavailable to the non-interactive command; retain **FAIL/DEGRADED** and perform separate GET-only verification only after a successful authenticated deployment.

## August 22, 2026 Control-5 correction: explicit filtered-run aliases — FAIL/DEGRADED

The live observability defect was that GET `/api/runs?trigger=daytrading_cron` and `trigger=reconciliation_cron` could return canonical `run_log` rows without an explicit indication that an alias filter had been requested, making filtered live history ambiguous. This smallest reliability-only correction changes only `src/api.ts`: existing SQL alias filtering is retained, alias responses add stable `trigger_alias` values (`daytrading_cron` or `reconciliation_cron`), and canonical `trigger` remains `cron` or `reconcile_cron`; canonical requests do not receive invented aliases. No schema, DDL, broker access, broker call, schedule, cap, authority, or trading behavior changed.

Required validation was focused read-only alias/canonical regression coverage, full `bun test`, `bunx tsc --noEmit`, and `git diff --check`; results were **11 tests / 101 assertions passed**, **157 tests / 520 assertions passed**, typecheck passed, and diff-check passed. Deployment is blocked by Wrangler's missing `CLOUDFLARE_API_TOKEN` (`In a non-interactive environment, it's necessary to set a CLOUDFLARE_API_TOKEN environment variable for wrangler to work.`); no deployment or broker-mutating endpoint was used. Production remains **FAIL/DEGRADED**, not healthy, until authenticated deployment and separate GET-only live verification succeed.
