...
The canonical release version is **`2.6.0`**, established by the deployable source's `schema.sql` `bot_config.version` seed and the existing dashboard footer. `package.json`, the Worker health response, the dashboard release marker/footer, and the version regression now agree on `2.6.0`. The health response reads the shared `src/version.ts` constant; no runtime configuration value is changed, and `/api/config` remains the raw D1 configuration response. This is a reliability-only observability correction: no trading logic, schedules, caps, edge gates, broker calls, D1 mutation semantics, or endpoint methods changed.

Validation for this correction: `bun test test/release-version.test.ts test/dashboard-readonly.test.ts` — **12 tests passed, 106 expect() calls**; `bun test` — **157 tests passed, 518 expect() calls**; `bunx tsc --noEmit` passed; `git diff --check` passed; and `bunx wrangler deploy --dry-run --outdir /tmp/alpaca-trading-bot-version-dry-run` passed with a 281.23 KiB upload preview and no deployment. No external or broker-mutating endpoint was used for this work.
