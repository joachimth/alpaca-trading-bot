...
The exact current local Alpaca source is HEAD `2fd04d6b416eee03fa56987c6627b83bc180ba7f` on `fix/remove-premature-position-upsert-entryside`, release **2.6.0**. This current HEAD supersedes stale historical references to `1d14eb9` and `1c6914d` in older control records; those hashes remain historical evidence only. Live deployment identity is not proven: `/health` reports **1.0.0**, `/api/config.version` reports **2.4.0**, and `bunx wrangler whoami` is blocked by **You are not authenticated. Please run `wrangler login`.**
