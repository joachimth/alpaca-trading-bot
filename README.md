## Monday, August 24, 2026 Control-71 strict read-only production control - LOCAL CORRECTION / LIVE OPEN FAIL-DEGRADED

Control-71 used only GET requests against `/health`, `/api/config`, `/api/dashboard`, `/api/positions`, `/api/runs`, and `/api/trades`; all six returned HTTP 200 JSON. Production remains **OPEN FAIL/DEGRADED**, not healthy: live health/config are **1.0.0/2.4.0** versus local release **2.6.0**. Positions remain explicitly broker-authoritative (`positionsAvailable=true`, `source=alpaca`, 26 broker rows); current account equity is approximately **98401.66 USD** versus `last_equity=98504.5039` (down by comparison), with an unexplained account/latest-snapshot difference.

The four local UTC schedules remain daytrading `*/5 13-21 * * 1-5`, swing `0 22 * * 1-5`, crypto `7-59/30 * * * *` near `:07/:37`, and reconciliation `*/10 * * * *`; live delivery includes daytrading run 3331, reconciliation run 3329 (`MAINTENANCE_ONLY`), and crypto run 3325, while no fresh August 24 swing run or explicit `CYCLE_LEASE_HELD` record is proven. Live errors include PLUG minimum-order rejection, Cloudflare subrequest exhaustion, and broker/internal mismatches. Filled lifecycle fields exist, but exact per-fill `gross`/`fee`/`net` remain null under `unavailable_fill_lot_exact`; some filled sells have `estimated_value=0` despite nonzero notional.

The confirmed PLUG rejection reproduces locally as a narrow daytrading BUY preflight gap. Control-71 adds only a broker-minimum-order preflight with structured `MIN_ORDER_SIZE` skip observability and regression coverage; SELL/CLOSE/protective exits, caps **5000/3700/2000 USD**, schedules, thresholds, sizing intent, fee attribution, crypto edge gates, broker authority, and trading behavior remain unchanged. Dashboard crypto fee telemetry remains stale/conflicting (`available` as of August 18 while August 24 crypto runs report `FEE_DATA_UNAVAILABLE`), swing exposure exceeds the displayed 3700 cap with semantics unresolved, live filters/pagination/aliases/candidate counters and crypto edge-gate wiring remain unproven, and release provenance remains blocked by unauthenticated Wrangler. See `CORRECTION_WORK_ITEM_2026-08-24_CONTROL-71.md`.
