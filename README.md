## Monday, August 24, 2026 Control-72 strict read-only production control - DOCUMENTATION CORRECTION / LIVE OPEN FAIL-DEGRADED

Control-72 used only GET requests against `/health`, `/api/config`, `/api/dashboard`, `/api/positions`, `/api/runs`, and `/api/trades`; all six returned HTTP 200 JSON. Production remains **OPEN FAIL/DEGRADED**, not healthy: live health/config are **1.0.0/2.4.0** versus local release **2.6.0**, and the active Worker cannot be mapped to the checked-out commit because Wrangler is unauthenticated. Positions remain explicitly broker-authoritative (`positionsAvailable=true`, `source=alpaca`, 26 broker rows); current account equity is **98391.48 USD** versus `last_equity=98504.5039`, a downward difference of **113.0239 USD**.

The four local UTC schedules remain daytrading `*/5 13-21 * * 1-5`, swing `0 22 * * 1-5`, crypto `7-59/30 * * * *` near `:07/:37`, and reconciliation `*/10 * * * *`; fresh live delivery includes daytrading run **3346** at `18:01:37`, reconciliation run **3345** at `18:01:19` with structured `MAINTENANCE_ONLY`, and crypto run **3338** at `17:38:15`. No fresh August 24 swing run or explicit `CYCLE_LEASE_HELD` record is proven. Run `code=LEASE_HELD` and `search=LEASE` probes returned the same unfiltered page, while live run rows omit the durable candidate counters that local source/tests implement.

The active live artifact still contains daytrading run **3344** with the Alpaca minimum-order `40310000` rejection for NIO, plus historical Cloudflare subrequest exhaustion in swing run **3182**. Trade lifecycle IDs, quantities, statuses, and timestamps are present, but sampled filled rows retain `gross=null`, `fee=null`, and `net=null` under `accounting_status=unavailable_fill_lot_exact` and `fee_attribution=none-recorded`; crypto aggregate accounting remains arithmetically consistent at gross **-56.616426**, fees **269.110169**, net **-325.726595**, while exact per-fill attribution remains unavailable. Current trade `status=filled` sampling is filled-only, but the live endpoint omits filter metadata and `offset=10`/`page=2` repeat the first-page IDs, so complete filter/pagination behavior is not certified.

The checked-out code already contains the narrow daytrading BUY minimum-notional preflight, broker-authoritative position projection, bounded reconciliation, structured skip observability, exact four-schedule dispatch, capital-cap resolution, conservative accounting, and fail-closed crypto fee/calibrated-edge gates. Normal crypto signal generation has no calibrated `rawEdgeBps` producer, so the local edge gate correctly rejects missing calibration; live edge-gate wiring is not independently visible on the old artifact. Caps remain exactly **5000/3700/2000 USD**, with no trading-behavior change. Control-72 is documentation/status-only because the remaining production defect is release drift; focused **90/461**, full **204/775**, typecheck, and diff-check pass locally. See `CORRECTION_WORK_ITEM_2026-08-24_CONTROL-72.md`.
