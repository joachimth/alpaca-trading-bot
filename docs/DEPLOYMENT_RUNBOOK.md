## August 21, 2026 bounded broker-ledger subrequest correction — release gate, not deployed
...
Release gates passed: focused `bun test test/entry-position-authority.test.ts test/alpaca-activities.test.ts test/broker-ledger.test.ts test/order-reconciliation.test.ts` (**24 tests / 90 assertions**), full `bun test` (**149 tests / 470 assertions**), `bunx tsc --noEmit`, `git diff --check`, and `bunx wrangler deploy --dry-run`. **Deployment state: NOT DEPLOYED; an authorized Cloudflare credential is available in the secure vault and deployment is required under the standing maintenance rule.** After deployment, use only GET/read-only verification first and wait for natural scheduled evidence; never manually trigger a cycle or call submit/cancel/close/replace/retry endpoints.
