## Control-72 release gate - August 24, 2026

Keep production **OPEN FAIL/DEGRADED**. Strict GET-only checks of `/health`, `/api/config`, `/api/dashboard`, `/api/positions`, `/api/runs`, and `/api/trades` all returned HTTP 200 JSON, but live identity is health **1.0.0** and config **2.4.0** versus checked-out release **2.6.0**. Wrangler is unauthenticated, so the active deployment cannot be mapped to the checked-out commit. Broker positions are authoritative (`positionsAvailable=true`, `source=alpaca`, 26 rows); equity is **98391.48** versus `last_equity=98504.5039`, down **113.0239 USD**. Configured caps remain **5000/3700/2000 USD**.

Local four-lane schedules remain daytrading `*/5 13-21 * * 1-5`, swing `0 22 * * 1-5`, crypto `7-59/30 * * * *` (`:07/:37`), and reconciliation `*/10 * * * *`. Fresh live runs prove daytrading **3346**, reconciliation **3345** (`MAINTENANCE_ONLY`), and crypto **3338** near the expected cadence; no fresh August 24 swing run or lease-held event is proven. Live defects/gaps include run **3344** minimum-order 403, historical Cloudflare subrequest exhaustion, absent candidate counters, repeated trade pagination pages, unavailable exact per-fill gross/fee/net, and unexposed live crypto edge-gate results.

The checked-out release already contains the narrow daytrading BUY minimum-notional preflight, structured skip observability, broker-authoritative position projection, bounded reconciliation, cap aliases, conservative accounting, and fail-closed crypto fee/calibrated-edge gates. Normal crypto signal generation has no calibrated edge producer, and tests verify missing edge is rejected without invention; do not add a producer or alter trading behavior in this correction. Preserve broker authority, caps, schedules, thresholds, sizing, fee freshness, edge policy, order semantics, and trading behavior. No trigger, submit, cancel, close, replace, retry, migration, deployment, or broker-mutating smoke test is allowed.

Local focused **90/461**, full **204/775**, typecheck, and diff-check passed. Deployment remains blocked by the exact Wrangler error `You are not authenticated. Please run wrangler login.` Restore credentials securely, identify the active deployment, deploy only an authorized clean reliability artifact if still required under the standing maintenance rule, and immediately perform a separate GET-only verification. See `CORRECTION_WORK_ITEM_2026-08-24_CONTROL-72.md`.
