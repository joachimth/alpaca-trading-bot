## August 22, 2026 Control-18 release gate - FAIL/DEGRADED

The strict control at **2026-08-22 23:00:16-23:00:17 UTC** used only GET requests. `/health`, `/api/config`, `/api/dashboard`, `/api/positions`, `/api/runs`, and `/api/trades` all returned HTTP 200, but release acceptance remains **FAIL/DEGRADED**. Live identity is unresolved: `/health=1.0.0`, persisted config `2.4.0`, versus local deployable HEAD `131898b9e4cab3544ae9b793123c1c86d5763cdc`, version `2.6.0`.

Read-only evidence confirms broker-authoritative positions (`positionsAvailable=true`, `source=alpaca`, 29 rows), equity `98,504.50` versus `last_equity=98,504.5039` with `change_today=0` and material direction unverified, and unchanged caps `$5,000/$3,700/$2,000`. Local source retains all four UTC schedules: daytrading `*/5 13-21 * * 1-5`, swing `0 22 * * 1-5`, crypto `7-59/30 * * * *`, reconciliation `*/10 * * * *`; active deployed schedule identity cannot be authenticated.

Reconciliation is fresh through run `2889` at `23:00:53` as `MAINTENANCE_ONLY`. Crypto is fresh near `:07/:37`, including `22:07:57` and `22:37:56`. Daytrading is stale, newest filtered run `2556` at `2026-08-20 21:55:24` with `CYCLE_LEASE_HELD`; swing is stale, newest known run `2200` at `2026-08-18 22:00:36` with divergence and `RISK_HALTED`. No fresh weekday delivery is proven for either stock strategy.

The live old response omits local `trigger_alias` annotations and `run_log` does not persist analyzed/filtered candidate counts. Trade lifecycle fields exist, but sampled filled rows have null `gross`, `fee`, and `net` under `unavailable_fill_lot_exact`; exact fill-lot accounting is not demonstrable. Local crypto fee/calibrated-edge gates remain fail-closed and live fee-unavailable skips support that behavior, but positive-edge producer evidence is unavailable.

No code defect was isolated. Control-18 is documentation/status-only, with focused 78 tests / 327 assertions, full 168 tests / 584 assertions, typecheck, and diff checks passing. Deployment gate remains the exact Wrangler response **`You are not authenticated`**. Do not use `wrangler deploy --temporary`; restore authenticated access, inspect active provenance and schedules, obtain separate deployment authorization, deploy only if still required, and then perform separate GET-only release and natural weekday verification. No trigger, order, cancellation, close, replace, retry, migration, deployment, or broker mutation was used. Separate live evidence: `/workspace/alpaca-control-18-live-20260822T230209Z/`.

## August 22, 2026 Control-18 strict read-only production control - FAIL/DEGRADED

At **2026-08-22 23:00:16-23:00:17 UTC**, all six required production GET endpoints returned HTTP 200. Acceptance remains **FAIL/DEGRADED**, not healthy: `/health=1.0.0`, `/api/config.version=2.4.0`, local HEAD `131898b9e4cab3544ae9b793123c1c86d5763cdc`, and deployable version `2.6.0` do not establish active Worker/source identity.

Read-only evidence: positions `positionsAvailable=true`, `source=alpaca`, 29 rows; equity `98504.50` versus `last_equity=98504.5039`, about `-0.0039`, with `change_today=0` so material direction cannot be verified; caps exactly `5000/3700/2000`. Local UTC schedules remain daytrading `*/5 13-21 * * 1-5`, swing `0 22 * * 1-5`, crypto `7-59/30 * * * *`, and reconciliation `*/10 * * * *`, but active deployed four-schedule identity is unresolved.

Fresh reconciliation runs `2888` at `22:50:50` and `2887` at `22:40:49` are `MAINTENANCE_ONLY`; fresh crypto runs `2886` at `22:37:56`, `2882` at `22:07:57`, `2878` at `21:37:56`, and `2874` at `21:07:57` are around `:07/:37` with structured skips. Saturday supplies no fresh daytrading/swing proof; historical lease/error/risk skips remain. `/api/trades` returns 50 filled rows with `submitted_at`/`filled_at`, null inapplicable terminal fields, null gross/fee/net, `accounting_status=unavailable_fill_lot_exact`, and `fee_attribution=none-recorded`; aggregate arithmetic is not exact fill-lot proof. The live old response omits `trigger_alias`, and `run_log` lacks durable analyzed/filtered counts.

No code defect was isolated. Do not change trading/reliability code, configuration, caps, schedules, or edge gates. Local filtered-alias and crypto fee/`rawEdgeBps` fail-closed wiring/tests pass, while live positive calibrated-edge producer evidence is unavailable. Wrangler is blocked by **`You are not authenticated`**. Restore authenticated Wrangler, inspect active provenance/schedules, obtain separate deployment authorization, deploy only if required, then run separate GET-only verification. No trigger, submit, cancel, close, replace, retry, migration, deployment, or broker mutation was used. See `CORRECTION_WORK_ITEM_2026-08-22_CONTROL-18.md`.
