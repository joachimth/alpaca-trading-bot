...
Live defect: alias-filtered GET `/api/runs` history was not explicit enough for operations to distinguish `daytrading_cron` from its canonical stored `cron` trigger or `reconciliation_cron` from `reconcile_cron`. Exact correction scope is `src/api.ts` only: retain the existing SQL alias filtering, annotate alias-filtered rows with stable `trigger_alias`, and leave canonical `trigger` values and canonical-request responses unchanged. No deployment artifact, schema, schedule, cap, broker authority, broker call, or trading behavior was changed.

Release validation required and completed locally: focused dashboard read-only regression (**11 passed / 101 assertions**), full `bun test` (**157 passed / 520 assertions**), `bunx tsc --noEmit` passed, and `git diff --check` passed. The focused test proves explicit `trigger_alias` values, unchanged canonical history, no DDL, and no broker access. Do not deploy until Wrangler credentials are restored: deployment is blocked by the missing `CLOUDFLARE_API_TOKEN` with the exact error `In a non-interactive environment, it's necessary to set a CLOUDFLARE_API_TOKEN environment variable for wrangler to work.` Production remains **FAIL/DEGRADED**; after credentials are restored, require authenticated deployment plus separate GET-only verification of canonical and alias run filters before any health claim.
