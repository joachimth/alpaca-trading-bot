...
The exact current checkout is branch `fix/remove-premature-position-upsert-entryside`, HEAD `9f09b145d922597d1e0b7c80bb827a768c147706`, release **2.6.0**. The runtime-bearing reliability commit is `20d80ac87e08271fb0d9c1c7ea1027b72eebd48d`; later commits are documentation/status records. Live deployment identity remains unproven: `/health` reports **1.0.0**, `/api/config.config.version` reports **2.4.0**, the live config has no release identity, and Wrangler authentication is blocked by **`You are not authenticated. Please run \`wrangler login\`.`**
