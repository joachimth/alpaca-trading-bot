## Control-71 release gate - August 24, 2026

Keep production **OPEN FAIL/DEGRADED**. Strict GET-only checks of `/health`, `/api/config`, `/api/dashboard`, `/api/positions`, `/api/runs`, and `/api/trades` all returned HTTP 200 JSON, but live identity is health **1.0.0** and config **2.4.0** versus checked-out release **2.6.0**. Broker positions are authoritative (`positionsAvailable=true`, `source=alpaca`, 26 rows); equity is approximately **98401.66** versus `last_equity=98504.5039` (down by comparison). Configured caps remain **5000/3700/2000 USD**.

Local four-lane schedules remain daytrading `*/5 13-21 * * 1-5`, swing `0 22 * * 1-5`, crypto `7-59/30 * * * *` (`:07/:37`), and reconciliation `*/10 * * * *`. Fresh live runs prove daytrading 3331, reconciliation 3329 (`MAINTENANCE_ONLY`), and crypto 3325; no fresh August 24 swing run or lease-held event is proven. Live defects/gaps include PLUG minimum-order 403, Cloudflare subrequest exhaustion, broker/internal mismatches, swing exposure approximately **4805.85** above the displayed **3700** cap with semantics unresolved, null exact per-fill gross/fee/net, zero filled-sell estimates, stale/conflicting crypto fee telemetry, unexposed crypto edge-gate wiring, and unproven live filtering/pagination/aliases/candidate counters.

Control-71 adds only a local daytrading BUY broker-minimum-order preflight with structured `MIN_ORDER_SIZE` skip evidence and focused regression coverage. Preserve broker authority, caps, schedules, thresholds, sizing, fee freshness, crypto edge policy, order semantics, and trading behavior; do not use trigger, submit, cancel, close, replace, retry, migration, or other broker-mutating smoke tests. Wrangler remains blocked by `You are not authenticated. Please run wrangler login.` No deployment is performed unless a clean authorized release becomes possible, followed by a separate GET-only verification. See `CORRECTION_WORK_ITEM_2026-08-24_CONTROL-71.md`.
