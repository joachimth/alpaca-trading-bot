...
Validation: `bun test test/release-version.test.ts test/dashboard-readonly.test.ts` — **12 tests passed, 106 expect() calls**; full `bun test` — **157 tests passed, 518 expect() calls**; `bunx tsc --noEmit` passed; `git diff --check` passed; and `bunx wrangler deploy --dry-run --outdir /tmp/alpaca-trading-bot-version-dry-run` passed with no deployment. No external or broker-mutating endpoint was used. The prior August 22, 2026 deployment receipt documents an earlier correction and is not evidence that this local version fix is deployed.
