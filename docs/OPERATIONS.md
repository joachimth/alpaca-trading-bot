...
The live defect was ambiguous filtered run observability: GET `/api/runs` requests using `daytrading_cron` or `reconciliation_cron` could return canonical rows whose response did not explicitly identify the requested alias. The correction is limited to the read-only API boundary in `src/api.ts`: preserve the existing SQL alias predicates and canonical stored values (`cron` and `reconcile_cron`), while adding `trigger_alias` only to alias-filtered responses. Canonical requests remain unchanged and do not get an invented alias. There was no DDL, migration, broker access, broker call, schedule, cap, broker-authority, or trading-logic change.

Validation required and completed: focused `test/dashboard-readonly.test.ts` alias/canonical/no-broker/no-DDL checks (**11 passed / 101 assertions**), full `bun test` (**157 passed / 520 assertions**), `bunx tsc --noEmit` passed, and `git diff --check` passed. Deployment was not performed because Wrangler is blocked by the missing `CLOUDFLARE_API_TOKEN` and reports: `In a non-interactive environment, it's necessary to set a CLOUDFLARE_API_TOKEN environment variable for wrangler to work.` Keep production **FAIL/DEGRADED** until authenticated deployment and separate GET-only live verification prove the correction.
